...
import { Button, Card, FormControl, FormControlLabel, Link, MenuItem, Popover, Radio, RadioGroup, Select } from "@mui/material";
...
const UserMonetisation = ({ analyticsWebId, globalAlert, setViewVisitorProfile }) => {
	...
	const handleUserClick = async (session, index) => {
	...
	const handleResetState = () => {
		...
		setVisitLog([]);
		setFilterData({ "filterType": "ALL", "dateFilter": "LAST_7_DAYS", "startDate": new Date(new Date().setDate(new Date().getDate() - 7)), "endDate": new Date(), "dateFilterVisitLog": "LAST_7_DAYS", "startDateVisitLog": new Date(new Date().setDate(new Date().getDate() - 7)), "endDateVisitLog": new Date() });
	};
									...
									>
										<div className="p-3">
											...
											>
												...
												<FormControlLabel value="USER" control={<Radio />} label="Identified" />
												<FormControlLabel value="DEVICE" control={<Radio />} label="Anonymous" />
											</RadioGroup>
						...
						<tbody>
							{
								sessions.length > 0 ?
									sessions.map((session, index) => {
										return (
											<tr key={index} className={selectedIndex === index ? "active" : ""}>
												<td className="cursor-pointer overflow-x-hidden text-overflow-ellipsis" style={{ fontSize: "0.8rem" }} onClick={() => { handleUserClick(session, index) }}>
													{session.name}
												</td>
											</tr>
										)
						...
						<>
							...
							>
								<div className="p-3">
									...
									<div className="d-flex justify-content-center mt-3">
										<Button variant="contained" color="primary" onClick={()=>{handleUserClick(sessions?.[selectedIndex], selectedIndex); handleCloseVisitLog();}}>
											Apply
										</Button>
									</div>
								</div>
						...
						<Card className="p-2 mb-3" key={index}>
							<Row className="p-1">
								...
								<Col sm={2} style={{ fontSize: "0.8rem" }}>
									{ sessions?.[selectedIndex]?.type !== "DEVICE" &&
										<div className="d-flex align-items-center">
											<i className="fas fa-address-card mr-2" style={{ fontSize: "1.5em" }}></i>
											<Link component="a" className="cursor-pointer" onClick={() => { handleClickViewVisitorProfile(sessions?.[selectedIndex]?.groupId, sessions?.[selectedIndex]?.userId); }}>
...
export default connect(null, mapDispatchToProps)(UserMonetisation);