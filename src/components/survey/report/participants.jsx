...
import { getSurveyReportParticipant, surveyReportDataBrowser } from "../../../services/surveyService"
...
const Participants = ({ id, globalAlert, isAnimated }) => {
    ...
    useEffect(() => {
        const data = `sryId=${id}`
        getSurveyReportParticipant(data).then(res => {
            if (res?.status === 200) {
    ...
    useEffect(() => {
        ...
        surveyReportDataBrowser(data).then(res => {
            if (res?.status === 200) {
    ...
    return (
        <>
            ...
            <h4>Survey : {participantsData?.surveyName}</h4>
                {
                    participantSelected?
