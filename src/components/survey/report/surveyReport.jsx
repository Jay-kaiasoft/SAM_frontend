...
const SurveyReport = ({ location, setLoader, globalAlert }) => {
    ...
    const handlePrint = async ()=>{
        ...
        setTimeout(async () => {
            ...
            let canvasData = {"questionsData":questionsData, "countryData":countryData, "reportType":"Survey"};
    ...
    const handleCsv = () => {
        ...
        surveyReportDataBrowser(dataQueAnsList).then(res => {
            if (res.status === 200) {
                genrateCsv("Survey", res.result.surveyName, res.result.questions, res.result.countryList);
                setLoader({
                    load: false
                })
            } else {
                    ...
                    <Col xs={12}>
                        <div className="d-flex align-items-center">
                            <div className="icon-wrapper mr-3">
                                <Link component="a" className="btn-circle" data-toggle="tooltip" title="Back" onClick={()=>{history.push("/managesurvey")}}>
...
export default connect(null, mapDispatchToProps)(SurveyReport);