...
import {deleteSurveyCategory, getSurveyCategoryListPages, saveSurveyCategory} from "../../services/surveyService";
...
const ManageSurveyCategory = ({subUser,globalAlert,confirmDialog}) => {
    ...
    const handleClickDelete = (id) => {
        confirmDialog({
            ...
            onConfirm: () => {
                ...
                deleteSurveyCategory(requestData).then(res => {
                    if (res.status === 200) {
                        globalAlert({
                            type: "Success",
                            text: res.message,
    ...
    const displayCategory = useCallback(()=>{
        ...
        getSurveyCategoryListPages(data).then(res => {
            if (res.result) {
                setData(res.result.surveyCategoryList);
                setTotalPages(res.result.getTotalPages);
                setTotalData(res.result.totalSurveyCategory);
            }
        });
        ...
        <Row className="midleMain">
            <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                <div className="icon-wrapper d-inline-block mr-5">
                    <Link component="a" className="btn-circle" data-toggle="tooltip" title="Back" onClick={()=>{history.push("/mysurveytemplates")}}>
                        <i className="far fa-long-arrow-left"></i>
                        <div className="bg-dark-grey"></div>
                    </Link>
                </div>
                <h3 className="d-inline-block mb-0 align-middle">Survey Category</h3>
                <div className="icon-wrapper d-inline-block mx-5">
                    <CheckPermissionButton module="survey category" action="add">
                                            ...
                                            <td className="text-center">
                                                {
                                                    dataV.memberId !== 0 &&
                                                    <>
                                                        <CheckPermissionButton module="survey category" action="edit">
                                                            <i className="far fa-pencil-alt mr-3" data-toggle="tooltip" title="Edit" onClick={()=>{handleClickEdit(dataV.id)}}></i>
                                                        </CheckPermissionButton>
                                                        <CheckPermissionButton module="survey category" action="delete">
...
export default connect(mapStateToProps,mapDispatchToProps)(ManageSurveyCategory);