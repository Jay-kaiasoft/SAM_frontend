...
import {getCampaignsReportPrintAB} from "../../../../services/emailCampaignService";
...
const CampaignReportPdfABTesting = ({location,globalAlert})=>{
    ...
    const createPdf = useCallback(()=>{
        ...
        pdfMake.createPdf(documentDefinition).download(`emailcampaignABtestingreport.pdf`);
        setTimeout(function (){
            window.close();
        },1000);
    }, []);
    const executePrint = useCallback((data,canvasData) => {
        ...
        if(data.productLinksB.length > 0){
            temp = data.productLinksB.map((value, i)=>(
                [
                    {text: i+1},
                    {text: value.campLink, alignment: "left"},
                    {text: value.uniqueClicks},
        ...
        if(data.productLinksWinner.length > 0){
            temp = data.productLinksWinner.map((value, i)=>(
                [
                    {text: i+1},
                    {text: value.campLink, alignment: "left"},
                    {text: value.uniqueClicks},
        ...
        if(data.membersB.length > 0){
            temp = data.membersB.map((value, i)=>([
                {text: i+1},
                {text: value.firstName},
                {text: value.lastName},
                {text: value.email},
        ...
        if(data.membersWinner.length > 0){
            temp = data.membersWinner.map((value, i)=>([
                {text: i+1},
                {text: value.firstName},
                {text: value.lastName},
                {text: value.email},
        ...
        if(data.sourceLinksB.length > 0){
            temp = data.sourceLinksB.map((value, i)=>([
                {text: i+1},
                {text: value.campLink, alignment: "left"},
                {text: value.pc},
                {text: value.mobile},
        ...
        if(data.sourceLinksWinner.length > 0){
            temp = data.sourceLinksWinner.map((value, i)=>([
                {text: i+1},
                {text: value.campLink, alignment: "left"},
                {text: value.pc},
                {text: value.mobile},
    ...
    useEffect(()=>{
        ...
        getCampaignsReportPrintAB(requestData).then(res => {
            if (res?.status === 200) {
...
export default connect(null, mapDispatchToProps)(CampaignReportPdfABTesting);