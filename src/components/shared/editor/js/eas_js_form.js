...
import {socialfollow, droparraycommon, drop_link, droparraysettingcommon, loadeverytimecommon, loadeverytimecommonformtype, replaccon, unprotectSource, emtmsedisplay, showdsm, hidedsm, mySlideronc, refleftimageandcaption, bigimagesetting, closeleanModal, browseimagebtnreload, blockdelete, tbfcontrol, dvdcontrol, imagescontrol, socfollowcontrol, imgcardcontrol, multiimgcardcontrol, mulimgcapcontrol, mulimgonecapcontrol, videocontrol, linkcontrol, blocksettingcommon, dividerSettingCommon, buttonSetting, editreplaceimagelinkcommon, blocksettingheadercommon, clickSelectItemCallCommon, formBlockSettingCommon, formBlockSettingCommon2, deleteSingleImage, formBlockControl, removeActive, resetCounter, pagesettingcommon, questioncontentsetting, getdrophtmlheader, getdrophtmlfooter, pageHeaderSettingCommon, pageFooterSetting, attachmentSetting} from "./eas_js_common.js";
import { createCaptcha } from '../../../../assets/commonFunctions';
...
let droparray = {
	...
	"yesNoBlockEditor": '{{question}} <div class="col-12 blockanswer"> <div class="row"> <div class="col-12"> <div class="d-flex justify-content-start w-95-p" style="margin-left:5%;"> <div class=""> <div style="width: 100px;height: 100px;cursor: not-allowed;" class="d-flex justify-content-center align-items-center p-3 border border-1 rounded symbol-1"> <i class="far fa-check-circle" style="font-size: 50px;cursor: not-allowed;"></i> </div> <p class="label-1 text-center">Yes</p> </div> <div class="ml-3"> <div style="width: 100px;height: 100px;cursor: not-allowed;" class="d-flex justify-content-center align-items-center p-3 border border-1 rounded symbol-2"> <i class="far fa-times-circle" style="font-size: 50px;cursor: not-allowed;"></i> </div> <p class="label-2 text-center">No</p> </div> </div> </div> </div> </div>',
	...
	"captchaBlockEditor":'<div class="col-12"><span class="counter" style="width:5%; display:inline-block; vertical-align: top;">{{counter}}</span><div style="width:95%; display:inline-block;"><div class="blockanswer"><div class="d-flex align-items-center"><canvas id="capCodeEditor" class="capcode" width="300" height="80"></canvas><i class="far fa-sync fa-2x ml-3"></i></div><input type="text" placeholder="Enter code" style="margin-bottom:10px; margin-top:10px; border: 1px solid #d2d2d2;" readonly /></div></div></div>',
	"signatureBlockEditor":'{{question}} <div class="col-12"><span style="width:5%; display:inline-block;"></span><div style="width:95%; display:inline-block;"><div class="blockanswer"><div class="w-100 border rounded bg-white" style="height:150px"></div></div></div></div>',
	"formPage": '<div id="templateBody{{i}}" item-value="0" item-transition="fade" question-style="all" class="templateBody d-flex"><div class="questionBlock w-100"><div class="containerEmptyMessage text-center">Drop Content Blocks Here</div></div></div>',
}
...
export function loadeverytime(){
	...
	$("#pdconuploadimg").unbind("click").click(function(){
		...
		setTimeout(function(){
			...
			clickSelectItemCallForm("preview_McBlock_pdconimg","pdconimg","pdconimg");
		},1000);
	});
	/* Page Background */
	/* DEFAULT SETTINGS */
	...
	$("#addpage").unbind("click").click(function (){
		...
		$("#addpagethumb").find("#thankYouPage").before(`<div class="pagethumb active" title="New Form Page" data-toggle="tooltip"><div class="fold"></div><span>${formpagecount}</span></div>`);
...
export function reloadfirst(dataFormType) {
	...
	if($("#preview-template").find(".templateBody").find(".questionBlock").find('[rolefor="captcha"]').length > 0){
		createCaptcha("capCodeEditor");
	}
		...
		drop: function( event, ui ) {
			...
			if(ui.draggable.hasClass("mojoBlockFormItem")) {
				...
				if(found === "captcha"){
					if($("#preview-template").find(".templateBody").find(".questionBlock").find('[rolefor="captcha"]').length !== 0){
						$("#clickError").attr("data-type","Error");
						$("#clickError").val("You can put only one captcha");
						$("#clickError").trigger("click");
						return false;
					}
				}
	...
	if($('#pageheader').find('div.row').find("#headerdimg img").hasClass("mcnImage")) {
		...
		$('.tpl-image-replace').unbind("click").click(function () {
			...
			if($('#pageheader').find('div.row').find("#headerdimg").hasClass("headerdimg")) {
				...
				setTimeout(function () {
					clickSelectItemCallForm("preview_McBlock_header", "header2", "header2");
				}, 1000);
			} else {
				filemanager("preview_McBlock_header","header","header");
				setTimeout(function(){
					clickSelectItemCallForm("preview_McBlock_header","header","header");
				},1000);
			}
		});
	}
...
/* File Manager */
function clickSelectItemCallForm(block_id,target_id,contentblockid) {
...
window.clickSelectItemCallForm=clickSelectItemCallForm;
	...
	$(`#${block_id}[rolefor="image_form"],#${block_id}[rolefor="image_with_text_form"]`).find(".bottom-control .addoption").unbind("click").click(function(e){
		...
		if($("#"+block_id).attr("rolefor")==="image_form" || $("#"+block_id).attr("rolefor")==="image_with_text_form") {
			...
			if ($("#" + block_id).attr("rolefor") === "image_form") {
				$("#" + block_id).find(".blockanswer div.row").append('<div class="col-3 pb-3 d-flex flex-column" item-value="' + i + '" style="min-width: 175px;">' + droparray["blankImageBlockEditor"] + '<div class="text-center selecttype"><input type="' + type + '" disabled readonly></div><div class="text-center"><i class="far fa-trash-alt"></i></div></div>');
			} else if ($("#" + block_id).attr("rolefor") === "image_with_text_form") {
				$("#" + block_id).find(".blockanswer div.row:first").append('<div class="col-12 pb-3" item-value="' + i + '" style="display: flex;align-items: center;justify-content: space-between;"><div class="selecttype" style="padding-right: 10px;"><input type="' + type + '" disabled readOnly/></div><div style="min-width: 145px;padding-right: 10px;width:40%;">' + droparray["blankImageBlockEditor"] + '</div><div style="padding-right: 10px;"><textarea style="height:auto;" class="textArea"></textarea></div><i class="far fa-trash-alt"></i></div>');
			}
			$("#" + block_id).find("div.mojoImageUploader").unbind("click").click(function (e) {
				...
				if($(this).find("img").hasClass("mcnImage")){
					...
					if(formType !== "traditional")
					{
						...
						$("#dsmcsetting").html("No setting available for traditional one page form...");
					}
...
function getdrophtmlform(key) {
	...
	let fullhtml= '<div id="'+ids+'" class="mojoMcBlock frm-block dojoDndItem focus" question-transition="'+qTransition+'" rolefor="'+key+'" ><div data-dojo-attach-point="containerNode" style="width: 100%;">';
	fullhtml+= droparray["blankformBlockEditor"];
	fullhtml=replaccon("{{formControl}}",droparray[links],fullhtml);
	if(key === "consent_agreement" || key === "sms_consent_agreement"){
		...
		if(key === "sms_consent_agreement"){
			tempquestionBlockEditor.innerHTML = fullhtml;
			tempquestionBlockEditor.querySelector("input.agreement").setAttribute("value","You are agreeing to receive SMS [campaign type] from [Brand name]");
			fullhtml = tempquestionBlockEditor.innerHTML;
		}
	...
	if(key==="open_ended" || key === "email" || key === "phone") {
	...
	if(key==="single_answer" || key==="single_answer_checkbox") {
		fullhtml=replaccon("{{answer}}",droparray["radioBlockEditor"]+droparray["radioBlockEditor"],fullhtml);
	}
	if(key==="single_answer_button" || key==="single_answer_combo") {
		fullhtml=replaccon("{{answer}}",'<select class="singleanswercombo" style="margin-bottom: 10px;border-radius: 0;display:none;"></select>'+droparray["buttonBlockEditor"]+droparray["buttonBlockEditor"],fullhtml);
	}
	if(key==="matrix"){
	...
	if(key === "image_form" || key === "image_with_text_form"){
		fullhtml=replaccon("{{image}}",droparray["blankImageBlockEditor"],fullhtml);
	}
	if(key === "label"){
		fullhtml=replaccon("{{textdetail}}",deftextcont,fullhtml);
	...
	fullhtml+='</div></div>';
	fullhtml=fullhtml.replace('<i class="far fa-list showhidelist text-black" style="padding:5px;"></i>','');
	return fullhtml;
}
/* Form Control */
...
function launchPixieEditor() {
	...
	let interval = setInterval(() => {
		...
		pixie3 = new window.Pixie({...config});
		pixie3.setConfig({image: imageIDFullPath})
	}, 1000);
}
/* Image Editor */