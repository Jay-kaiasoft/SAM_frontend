...
import {socialfollow, droparraycommon, drop_link, droparraysettingcommon, loadeverytimecommon, loadeverytimecommonformtype, replaccon, unprotectSource, getCategoryListBox, emtmsedisplay, showdsm, hidedsm, mySlideronc, refleftimageandcaption, bigimagesetting, closeleanModal, browseimagebtnreload, blockdelete, tbfcontrol, dvdcontrol, imagescontrol, socfollowcontrol, imgcardcontrol, multiimgcardcontrol, mulimgcapcontrol, mulimgonecapcontrol, videocontrol, linkcontrol, blocksettingcommon, dividerSettingCommon, buttonSetting, editreplaceimagelinkcommon, blocksettingheadercommon, clickSelectItemCallCommon, formBlockSettingCommon, formBlockSettingCommon2, deleteSingleImage, formBlockControl, removeActive, resetCounter, pagesettingcommon, questioncontentsetting, getdrophtmlheader, getdrophtmlfooter, pageHeaderSettingCommon, pageFooterSetting, attachmentSetting} from "./eas_js_common.js";
...
let droparray = {
	...
	"buttonBlockEditor": '<div class="col-12 p-0 d-flex align-items-center" style="margin-bottom:10px;" unique-id="{{uniqueId}}"><input type="text" placeholder="Enter description for this option" /><i class="far fa-trash-alt" style="padding-left:10px;"></i></div>',
	...
	"imageFBlockEditor":'{{question}}<div class="col-12 blockanswer"><span style="width:5%; display:inline-block;"></span><span style="width:95%; display:inline-block;"><div class="row"><div class="col-3 pb-3 d-flex flex-column" item-value="0" style="min-width: 175px;">{{image}}<div class="text-center selecttype"><input type="radio" disabled readonly /></div><div class="text-center"><i class="far fa-trash-alt"></i></div></div></div></span></div>',
	"imageWithTextBlockEditor":'{{question}}<div class="col-12 blockanswer"><span style="width:5%; display:inline-block;"></span><span style="width:95%; display:inline-block;"><div class="row"><div class="col-12 pb-3" style="display: flex;align-items: center;justify-content: space-between;" item-value="0"><div class="selecttype" style="padding-right: 10px;"><input type="radio" disabled readonly /></div><div style="min-width: 145px;padding-right: 10px;width:40%;">{{image}}</div><div style="padding-right: 10px;"><textarea style="height:auto;" class="textArea"></textarea></div><i class="far fa-trash-alt"></i></div></div></span></div>',
	...
	"yesNoBlockEditor": '{{question}} <div class="col-12 blockanswer"> <div class="row"> <div class="col-12"> <div class="d-flex justify-content-start w-95-p" style="margin-left:5%;"> <div class="" unique-id="'+uuidv4()+'"> <div style="width: 100px;height: 100px;cursor: not-allowed;" class="d-flex justify-content-center align-items-center p-3 border border-1 rounded symbol-1"> <i class="far fa-check-circle" style="font-size: 50px;cursor: not-allowed;"></i> </div> <p class="label-1 text-center">Yes</p> </div> <div class="ml-3" unique-id="'+uuidv4()+'"> <div style="width: 100px;height: 100px;cursor: not-allowed;" class="d-flex justify-content-center align-items-center p-3 border border-1 rounded symbol-2"> <i class="far fa-times-circle" style="font-size: 50px;cursor: not-allowed;"></i> </div> <p class="label-2 text-center">No</p> </div> </div> </div> </div> </div>',
	...
	"formPage": '<div id="templateBody{{i}}" item-value="0" item-transition="fade" question-style="all" page-category="{{cat}}" page-category-id="{{catId}}" page-category-color="{{catColor}}" page-unique-id="{{uniqueId}}" class="templateBody d-flex"><div class="questionBlock w-100"><div class="containerEmptyMessage text-center">Drop Content Blocks Here</div></div></div>',
}
let droparraysetting = {
	...
	"contentSettingBlockEditor": '<div class="form-group"><label>Question Layout</label><select id="questionStyle" class="mb-0"><option value="all">All</option><option value="horizontal">Horizontal</option><option value="vertical">Vertical</option></select></div><div class="form-group" id="qTran" style="display:none"><label>Question Transition</label><select id="questionTransition" class="mb-0"></select></div>',
}

export function loadeverytime(){
	loadeverytimecommon();
	$("#aicontrol div.em").unbind("click").click(function (e) {
        e.stopPropagation();
        $("#clickQuestionAiModal").trigger("click");
        setTimeout(()=>{
            $("#goal").unbind("change").change(function(){
                if($(this).val() === "Other"){
                    $("#goaldescription").val("");
                    $("#goaldescriptiondiv").show();
                } else {
                    $("#goaldescriptiondiv").hide();
                    $("#goaldescription").val("");
                }
            });
			$("#noofquestion").unbind("keypress").keypress(function(event){
				const charCode = event.charCode || event.keyCode;
				if (charCode < 48 || charCode > 57) {
					event.preventDefault();
				}  
			});
            $("#ask_questions").unbind("click").click(async function(){
                if($("#about").val() === ""){
                    $("#clickError").attr("data-type","Error");
                    $("#clickError").val("Please enter Tell me what is this survey about?");
                    $("#clickError").trigger("click");
                } else if($("#goal").val() === ""){
                    $("#clickError").attr("data-type","Error");
                    $("#clickError").val("Please enter What is the goal or objective of this survey?");
                    $("#clickError").trigger("click");
                } else if($("#noofquestion").val() === "" || $("#noofquestion").val() === "0"){
                    $("#clickError").attr("data-type","Error");
                    $("#clickError").val("Please enter How many questions do you want to ask?");
                    $("#clickError").trigger("click");
                } else {
					$("#clickLoader").val(true);
					$("#clickLoader").trigger("click");
                    try {
                        let prompt = "The question types are:\n";
                        prompt += "- open_ended\n";
                        ...
                        prompt += "Answers:\n";
                        prompt += "QuestionType:\n";
                        prompt += "\n\n create survey for " + $("#about").val();
                        if($("#noofquestion").val() !== 0 && $("#noofquestion").val() !== "0" && $("#noofquestion").val() !== ""){
                            prompt += ", "+$("#noofquestion").val()+" questions only";
                        }
                        prompt += "\nThis survey's goal is " + $("#goal").val() === "Other" ? $("#goaldescription").val() : $("#goal").val();
						...
						$("#clickLoader").trigger("click");
						$("#clickQuestionAiModal").trigger("click");
                    	addPage("1", "None", "#000000");
						await reloadfirst(surveyType,[],tempData,{});
                    } catch (e) {
                        $("#clickError").attr("data-type","Error");
	...
	$("#pdconuploadimg").unbind("click").click(function(){
		...
		setTimeout(function(){
			...
			clickSelectItemCallSurvey("preview_McBlock_pdconimg","pdconimg","pdconimg");
		},1000);
	});
	/* Page Background */
	/* DEFAULT SETTINGS */
...
export async function reloadfirst(stSurveyType, demographicQuestions, tempAigeneratedData, brandData) {
	surveyType = stSurveyType;
	...
	if(typeof tempAigeneratedData !== "undefined" && tempAigeneratedData.length > 0){
		if(typeof demographicQuestions !== "undefined" && demographicQuestions.length > 0){
			addPage("1", "None", "#000000");
		}
		for(let i=0; i<tempAigeneratedData.length; i++){
			let qType = tempAigeneratedData[i].QuestionType;
			if(tempAigeneratedData[i].QuestionType === "Multiple Choice (Single Answer)" || tempAigeneratedData[i].QuestionType === "multiple_choice_single_answer"){
				qType = "single_answer";
			} else if(tempAigeneratedData[i].QuestionType === "Multiple Choice (Multiple Answers)" || tempAigeneratedData[i].QuestionType === "multiple_choice_multiple_answers"){
				qType = "single_answer_checkbox";
			}
			let fullhtml = getdrophtmlform(qType);
			$("#templateBody"+selectedpage).find(".questionBlock").append(fullhtml);
			blockedtblocan(lastformid);
			blockdrag();
			resetCounter(formBlockSetting);
			emtmsedisplay();
			savefullcontent();
			$("#" + lastformid).find(".blockquestion input[type=text]").val(tempAigeneratedData[i].Question);
			$("#" + lastformid).find(".blockquestion input[type=text]").trigger("keyup");
			if(qType === "rating_box"){
				if(tempAigeneratedData[i].hasOwnProperty("Answers")){
					$("#" + lastformid).find(".blockanswer").find(".rating-text-a").val(tempAigeneratedData[i].Answers.MinLabel);
					$("#" + lastformid).find(".blockanswer").find(".rating-text-b").val(tempAigeneratedData[i].Answers.MaxLabel);
				}
			} else if(qType === "single_answer" || qType === "single_answer_checkbox") {
				for(let j=2; j<tempAigeneratedData[i].Answers.length; j++){
					$("#" + lastformid).find(".bottom-control").find(".addoption").trigger("click");
				}
				let j = 0;
				$("#" + lastformid).find(".singleAnswer").each(function(){
					$(this).val(tempAigeneratedData[i].Answers[j]);
					if(tempAigeneratedData[i].Answers[j].toLowerCase() === "other" || tempAigeneratedData[i].Answers[j].toLowerCase() === "others"){
						$(this).siblings(".addcommentspan").find(".addcomment").trigger("click");
					}
					j++;
				});
			}
	...
	if(typeof brandData.selectLogo !== "undefined" && brandData.selectLogo === "yes"){
		let found = "headerlayout5";
		let fullhtml = getdrophtmlheader(found,droparray);
		$("#pageheader").find("div.row").remove();
		$("#pageheader").append(fullhtml);
		emtmsedisplay();
		savefullcontent();
		hidedsm();
		pageHeaderSetting();
		let itempath = user?.brandKits?.filter((v) => v.brandName === brandData?.selectBrand)[0].brandLogo;
	...
	if($('#pageheader').find('div.row').find("#headerdimg img").hasClass("mcnImage")) {
		...
		$('.tpl-image-replace').unbind("click").click(function () {
			...
			if($('#pageheader').find('div.row').find("#headerdimg").hasClass("headerdimg")) {
				...
				setTimeout(function () {
					clickSelectItemCallSurvey("preview_McBlock_header", "header2", "header2");
				}, 1000);
			} else {
				filemanager("preview_McBlock_header", "header", "header");
				setTimeout(function () {
					clickSelectItemCallSurvey("preview_McBlock_header", "header", "header");
				}, 1000);
			}
		});
	}
...
/* File Manager */
function clickSelectItemCallSurvey(block_id,target_id,contentblockid) {
...
window.clickSelectItemCallSurvey=clickSelectItemCallSurvey;
...
function getdrophtmlform(key) {
	...
	if(typeof $("#templateBody"+selectedpage).attr("question-style") !== "undefined" && $("#templateBody"+selectedpage).attr("question-style") !== "all"){
	...
	if(key === "consent_agreement"){
		...
		fullhtml=replaccon("{{question}}",droparray["questionBlockEditor"],fullhtml);
	}
...
export function formBlockSetting(block_id) {
	formBlockSettingCommon(block_id,removeActive,"survey");
	formBlockSettingCommon2(block_id,removeActive,surveyType,droparraysetting,pagesetting,selectedpage,droparray,formBlockSetting);
	...
	$(`#${block_id}[rolefor="image_form"],#${block_id}[rolefor="image_with_text_form"]`).find(".bottom-control .addoption").unbind("click").click(function(e){
		...
		if($("#"+block_id).attr("rolefor")==="image_form" || $("#"+block_id).attr("rolefor")==="image_with_text_form") {
			...
			if ($("#" + block_id).attr("rolefor") === "image_form") {
				$("#" + block_id).find(".blockanswer div.row").append('<div class="col-3 pb-3 d-flex flex-column" item-value="' + i + '" style="min-width: 175px;">' + droparray["blankImageBlockEditor"].replaceAll("{{uniqueId}}",uuidv4()) + '<div class="text-center selecttype"><input type="' + type + '" disabled readonly></div><div class="text-center"><i class="far fa-trash-alt"></i></div></div>');
			} else if ($("#" + block_id).attr("rolefor") === "image_with_text_form") {
				$("#" + block_id).find(".blockanswer div.row:first").append('<div class="col-12 pb-3" item-value="' + i + '" style="display: flex;align-items: center;justify-content: space-between;"><div class="selecttype" style="padding-right: 10px;"><input type="' + type + '" disabled readOnly/></div><div style="min-width: 145px;padding-right: 10px;width:40%;">' + droparray["blankImageBlockEditor"].replaceAll("{{uniqueId}}",uuidv4()) + '</div><div style="padding-right: 10px;"><textarea style="height:auto;" class="textArea"></textarea></div><i class="far fa-trash-alt"></i></div>');
			}
		...
		deleteSingleImage(block_id,"survey",selectedpage);
...
function launchPixieEditor() {
	...
	let interval = setInterval(() => {
		...
		pixie2 = new window.Pixie({...config});
		pixie2.setConfig({image: imageIDFullPath})
	}, 1000);
}
/* Image Editor */