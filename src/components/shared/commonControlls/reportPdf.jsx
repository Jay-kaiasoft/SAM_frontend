...
const ReportPdf = ({user}) => {
    const createPdf = useCallback((reportType)=>{
        pdfMake.vfs = pdfFonts.vfs;
        pdfMake.createPdf(documentDefinition).download(`${reportType}Report.pdf`);
        setTimeout(function (){
            window.close();
        },3000);
    }, []);
    const executePrint = useCallback((canvasData) => {
        ...
        if(canvasData.questionsData.length > 0) {
            canvasData.questionsData.map((value)=>(
                documentDefinition.content.push({
                    image: value,
                    style: {alignment: "center"},
                    fit: [550, 700],
        ...
        if(canvasData.countryData.length > 0) {
            canvasData.countryData.map((value)=>(
                value.map((value1)=>(
                    documentDefinition.content.push({
                        image: value1,
                        style: {alignment: "center"},
        ...
        createPdf(canvasData.reportType);
    }, [createPdf,user]);
    useEffect(()=>{
        executePrint(JSON.parse(sessionStorage.getItem("canvasData")));
...
export default connect(mapStateToProps, null)(ReportPdf);