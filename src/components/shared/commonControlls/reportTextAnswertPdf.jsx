...
const ReportTextAnswertPdf = ({user}) => {
    ...
    const executePrint = useCallback((textAnswerData) => {
        ...
        documentDefinition.content.push(
            {text: `${textAnswerData.reportType} : ${textAnswerData.name}`, fontSize:10, margin:[0,10,0,0]},
            {text: `Question : ${textAnswerData.question}`, fontSize:10, margin:[0,10,0,0]},
        );
        let data = [];
        data.push([{text: 'No.', style: 'tableHeader'}, {text: 'Text Answers', style: 'tableHeaderLeft'}]);
        if(textAnswerData.answers.length > 0){
            textAnswerData.answers.forEach((value, index)=>(
                data.push([{text:index+1,alignment: "center"},{text:value.optionVal,alignment: "left"}])
            ))
        } else {
            data.push([{text: 'No Answers found', colSpan:2, alignment: "center"}, {}]);
        }
        documentDefinition.content.push(
        ...
        createPdf(textAnswerData.reportType);
    }, [createPdf,user]);
    useEffect(()=>{
        executePrint(JSON.parse(sessionStorage.getItem("textAnswerData")));
        sessionStorage.removeItem("textAnswerData");
    },[executePrint]);
    return (
        <div></div>
    );
...
export default connect(mapStateToProps, null)(ReportTextAnswertPdf);