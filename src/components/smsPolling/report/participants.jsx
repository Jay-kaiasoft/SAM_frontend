...
import { getSmsPollingReportParticipate, smsPollingReportDataBrowser } from "../../../services/smsPollingService"
...
const Participants = ({ id, globalAlert, isAnimated }) => {
    ...
    useEffect(() => {
        const data = `sid=${id}`
        getSmsPollingReportParticipate(data).then(res => {
            if (res?.status === 200) {
    ...
    useEffect(() => {
        if(participantSelected !== "") {
            const data = {
                ...
                "controlTypes": [],
                "questionNumbers": [],
                "toNumber": participantSelected
            }
            setSelectedParticipantData({});
            smsPollingReportDataBrowser(data).then(res => {
                if (res?.status === 200) {
    ...
    return (
        <>
            ...
            <h4>SMS Polling : {participantsData.smsPolling}</h4>
                {
                    participantSelected?
                ...
                <Col xs={12} sm={12} md={12} lg={3} xl={3} className="height-58 overflow-auto">
                    <div>
                        {participantsData?.participantsList?.map((participant) => {
                            return (
                                <div
                                    style={{ backgroundColor: `${participantSelected === participant?.toNumber ? "#F7F7F7" : ""}` }}
                                    className="d-flex border p-2 mb-2 rounded cursor-pointer"
                                    key={participant?.toNumber}
                                    onClick={() => { setParticipantSelected(participant?.toNumber) }}
                                >
                                    <div className="box-round">{participant?.firstName?.charAt(0)}{participant?.lastName?.charAt(0)}</div>
                                    <div className="d-flex flex-column ml-2">
                                        <span>{`${participant?.firstName} ${participant?.lastName}`}</span>
                                        <span>{`${participant?.fromCity !== "" ? participant?.fromCity+", " : ""}${participant?.fromState !== "" ? participant?.fromState+", " : ""}${participant?.fromCountry !== "" ? participant?.fromCountry : ""}`}</span>
                                    </div>
                                </div>
                            )
                        })}
