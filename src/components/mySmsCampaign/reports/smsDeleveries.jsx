...
import { getSmsCampaignsSmsReportsDeliveriesData } from "../../../services/smsCampaignService";
import DetailsModal from "./detailsModal";

const SmsDeleveries = ({ csId, smsId }) => {
    const [smsDeliveriesData, setSmsDeliveriesData] = useState([])
    ...
    const displaySmsDeliveriesList = useCallback(() => {
        let data = `csId=${csId}&smsId=${smsId}&searchKey=${searchSend}&page=${selectedPage}&size=${perPage}&sort=${sort}`;
        setSmsDeliveriesData([]);
        getSmsCampaignsSmsReportsDeliveriesData(data).then(res => {
            if (res?.status === 200) {
                if (res?.result) {
                    setSmsDeliveriesData(res?.result?.smsDeliveriesData);
    ...
    useEffect(() => {
        displaySmsDeliveriesList();
    }, [displaySmsDeliveriesList, csId, smsId, selectedPage, perPage, searchSend]);
    return (
        smsDeliveriesData?.length > 0 ?
            <Row>
                <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                    ...
                    <div className="table-content-wrapper height-58" style={{overflowY:"auto",overflowX:"hidden"}}>
                        <Table striped>
                            ...
                            <tbody>
                                {
                                    smsDeliveriesData?.map((dataValue, dataIndex) => {
                                        return (
                                            <tr key={dataIndex}>
                    ...
                    <Row className="mt-3">
                        <Col xs={6}><span className="align-middle pt-2">{`Showing ${smsDeliveriesData?.length > 0 ? (perPage * selectedPage) + 1 : 0} to ${((perPage * selectedPage) + 1) + smsDeliveriesData?.length - 1} of ${totalData} entries`}</span></Col>
