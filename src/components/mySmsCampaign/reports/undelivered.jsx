...
import { getSmsCampaignsReportsUndeliveredData } from "../../../services/smsCampaignService";

const Undelivered = ({ csId, smsId }) => {
    const [unDeliveredSmsData, setUnDeliveredSmsData] = useState()
    ...
    const displayUndeliveredList = useCallback(() => {
        let data = `csId=${csId}&smsId=${smsId}&searchKey=${searchSend}&page=${selectedPage}&size=${perPage}&sort=${sort}`;
        setUnDeliveredSmsData([]);
        getSmsCampaignsReportsUndeliveredData(data).then(res => {
            if (res?.status === 200) {
                if (res?.result) {
                    setUnDeliveredSmsData(res?.result?.unDeliveredSmsData);
                    setTotalPages(res?.result?.getTotalPages);
                    setTotalData(res?.result?.totalUndeliveredSms);
                }
            }
        });
    }, [searchSend, csId, smsId, selectedPage, perPage, sort]);
    useEffect(() => {
        displayUndeliveredList();
    }, [displayUndeliveredList, csId, smsId, selectedPage, perPage, searchSend]);
    return (
        unDeliveredSmsData?.length > 0 ?
            <Row className="mb-3">
                <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                    <h3 className="mb-3">Undelivered</h3>
                    ...
                    <div className="table-content-wrapper height-58" style={{overflowY:"auto",overflowX:"hidden"}}>
                        <Table striped>
                            ...
                            <tbody>
                                {
                                    unDeliveredSmsData?.length > 0 &&
                                    unDeliveredSmsData?.map((dataValue, dataIndex) => {
                                        return (
                                            <tr key={dataIndex}>
                    ...
                    <Row className="mt-3">
                        <Col xs={6}><span className="align-middle pt-2">{`Showing ${unDeliveredSmsData?.length > 0 ? (perPage * selectedPage) + 1 : 0} to ${((perPage * selectedPage) + 1) + unDeliveredSmsData?.length - 1} of ${totalData} entries`}</span></Col>
