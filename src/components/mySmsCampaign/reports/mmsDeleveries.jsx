...
import { getSmsCampaignsMmsReportsDeliveriesData } from "../../../services/smsCampaignService";

const MmsDeleveries = ({ csId, smsId }) => {
    const [mmsDeliveriesData, setMmsDeliveriesData] = useState([])
    ...
    const displayMmsDeliveriesList = useCallback(() => {
        let data = `csId=${csId}&smsId=${smsId}&searchKey=${searchSend}&page=${selectedPage}&size=${perPage}&sort=${sort}`;
        setMmsDeliveriesData([]);
        getSmsCampaignsMmsReportsDeliveriesData(data).then(res => {
            if (res?.status === 200) {
                if (res?.result) {
                    setMmsDeliveriesData(res?.result?.smsDeliveriesData);
                    setTotalPages(res?.result?.getTotalPages);
                    setTotalData(res?.result?.totalSmsDeliveries);
                }
            }
        });
    }, [searchSend, csId, smsId, selectedPage, perPage, sort]);
    useEffect(() => {
        displayMmsDeliveriesList();
    }, [displayMmsDeliveriesList, csId, smsId, selectedPage, perPage, searchSend]);
    return (
        mmsDeliveriesData?.length > 0 ?
            <Row className="mb-3">
                <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                    <h5 className="mb-3">MMS</h5>
                        ...
                        <Table striped>
                            <thead>
                                <tr>
                                    ...
                                    <th style={{ width: "15%" }} onClick={() => { handleClickSort("date", 4) }}>Date
                                        <span>
                            ...
                            <tbody>
                                {
                                    mmsDeliveriesData?.map((dataValue, dataIndex) => {
                                        return (
                                            <tr key={dataIndex}>
                    ...
                    <Row className="mt-3">
                        <Col xs={6}><span className="align-middle pt-2">{`Showing ${mmsDeliveriesData?.length > 0 ? (perPage * selectedPage) + 1 : 0} to ${((perPage * selectedPage) + 1) + mmsDeliveriesData?.length - 1} of ${totalData} entries`}</span></Col>
