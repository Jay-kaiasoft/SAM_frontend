...
const LogicFlow = ({ data, setData, categoryPageList, nodes, setNodes, onNodesChange, edges, setEdges, onEdgesChange }) => {
    ...
    const setLinkInSetData = useCallback((type, pageIndex, questionIndex, optionIndex, link, formName) => {
        ...
        if (radioTypes.includes(type) || imageTypes.includes(type)) {
            setData((prev) => {
                prev.atData.assessmentsPages[pageIndex].assessmentsQuestions[questionIndex].assessmentsOptions[optionIndex].formLink = link;
                prev.atData.assessmentsPages[pageIndex].assessmentsQuestions[questionIndex].assessmentsOptions[optionIndex].formName = formName;
                return prev;
            });
        } else if (ratingTypes.includes(type)) {
            setData((prev) => {
                prev.atData.assessmentsPages[pageIndex].assessmentsQuestions[questionIndex].formLinks[optionIndex].formLink = link;
                prev.atData.assessmentsPages[pageIndex].assessmentsQuestions[questionIndex].formLinks[optionIndex].formName = formName;
                return prev;
            });
        }
    }, [setData]);
    ...
    const onConnect = useCallback((params) => {
        ...
        if (sourceNode.type === "Radio") {
            ...
            setData(prev => {
                let sourceNext = prev.atData.assessmentsPages[sourceNode.data.pageIndex].assessmentsQuestions[sourceNode.data.questionIndex].next;
                if (targetNode.type === "End") {
                    ...
                    prev.atData.assessmentsPages[targetNode.data.pageIndex].assessmentsQuestions[targetNode.data.questionIndex].prev = { pageIndex: sourceNode.data.pageIndex, questionIndex: sourceNode.data.questionIndex };
                }
                prev.atData.assessmentsPages[sourceNode.data.pageIndex].assessmentsQuestions[sourceNode.data.questionIndex].next = sourceNext;
                return prev;
            });
    ...
    }, [nodes, data.atData, setNodes, setEdges, setData, setLinkInSetData, categoryPageList, edges?.length]);
    useEffect(() => {
        if (nodes.length !== 0) {
            let questions = extractQuestionsFromJson(data.atData);
            const nodesTemp = generateJSONForNode(questions, setLinkInSetData, categoryPageList);
            setNodes((prev) => {
                let temp = prev.map((e,index) => {
                    if (e.hasOwnProperty("data")) {
                        ...
                        if (e.data.hasOwnProperty("points")) {
                            e.data.points = e.data.points.map((eo,eindex)=>{
                                eo = nodesTemp[index].data.points[eindex];
                                return eo;
                            })
                        }
                            ...
                            categoryPageList.length > 0 ?
                                <>
                                    <div className="d-flex justify-content-between">
                                        ...
                                        <div className="d-flex justify-content-center lfCatTotal"><strong>Max PTs</strong></div>
                                    </div>
                                    <hr className="my-2" />
                                    {
                                        categoryPageList.map((v, i) => (
                                            <Fragment key={i}>
                                                <div className="d-flex justify-content-between cursor-pointer" onClick={() => { handleCategoryClick(v.catName) }}> 
                                                    ...
                                                    <div className="d-flex justify-content-center lfCatTotal">{v.maxPoints}</div>
                                                </div>
